pipeline {
    ...
    stages {
        stage('Build Server') {
            when {
                expression {
                    ...
                    sh(script: 'git diff --name-only HEAD | grep "^server/" || true', returnStatus: true) == 0
                }
            }
            steps {
        ...
        stage('Build Client') {
            when {
                expression {
                    def changedFiles = sh(script: 'git diff --name-only HEAD~1..HEAD | grep "^client/" || true', returnStdout: true).trim()
